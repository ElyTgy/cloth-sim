...
def make_edges_and_rests(w: int, h: int, dx: float, diagonals: bool = True) -> tuple:
    """
...
def spring_forces(pos: wp.array(dtype=wp.vec2),
    ...
    spring_force = -k * (current_length - rest[e]) * direction_unit
...
def integrate(pos: wp.array(dtype=wp.vec2),
    ...
    if verlet:
        # TODO: Implement Verlet integration method
        print("Verlet method not implemented. Using Euler method.")
...
def run_sim() -> list:
    ...
    for step in range(STEPS):
        ...
        wp.launch(spring_forces, dim=edges.shape[0], 
                 inputs=[pos, edges, rest, float(K), forces], device=DEVICE)
        
        # Calculate damping forces
        wp.launch(damping_forces, dim=W*H, 
                 inputs=[velocity, float(C), forces], device=DEVICE)
